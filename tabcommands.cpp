...
void TabView::onTabCommand(TabCommand command) {
    ...
    if (command == TabCommand::Instument)
        getMaster()->setComboBox(1,"instruments",240,5,200,30,pTab->changeTrackInstrument()); //Only UI feature
    else if (command == TabCommand::Panoram)
        getMaster()->setComboBox(6,"pan",570,5,50,30, pTab->changeTrackPanoram()); //Как и выше сбивает UI при отмене ввода
    else if (command == TabCommand::BPM) {
        auto newBpm = pTab->changeTrackBpm();
