...
void BendInput::setPtrBend(BendPoints *ptr) { ptrToBend = ptr; }
void BendInput::setPtrNote(Note *ptr) { ptrToNote = ptr; }

void ChangesInput::setPtrBeat(Beat *beatPtr) { ptrToBeat = beatPtr; }

bool MainView::isPlaying() { return _tabsView->getPlaying(); }

void MainView::onclick(int x1, int y1) {
  if (_currentView) _currentView->onclick(x1, y1);
}

void MainView::ondblclick(int x1, int y1) {
  if (_currentView) _currentView->ondblclick(x1, y1);
}

void MainView::onTabCommand(TabCommand command) {
  if (_currentView) _currentView->onTabCommand(command);
}

void MainView::onTrackCommand(TrackCommand command) {
  if (_currentView) _currentView->onTrackCommand(command);
}
...
void MainView::keyevent(std::string press) {
  ...
  if (_currentView) _currentView->keyevent(press);

  if (press == "esc") changeViewToLast();

  if ((press == CONF_PARAM("TrackView.quickOpen")) || (press == "quickopen")) {
    if (_tabsView->gotChanges() == false) {
      GmyFile gmyFile;
      std::string gfileName =
          std::string(AConfig::getInst().testsLocation) + "first.gmy";
  ...
  if (press == CONF_PARAM("Main.open")) {
    if (_tabsView->gotChanges() == false) {
      ...
      fd->setGeometry(0, 0, screen->geometry().width(),
      ...
      QString s;
      if (fd->exec()) s = fd->selectedFiles().at(0);
  ...
  if (press == "trackview") {
    changeCurrentView(
        _tabsView->getTracksViewRef()[_tabsView->getLastOpenedTrack()].get());
  }
  ...
  if (press == "newtab") {
    if (_tabsView->gotChanges() == false) {
      ...
      _tabsView->setTab(std::move(newTab));
      if (CONF_PARAM("skipTabView") == "1")
        _tabsView->onTabCommand(
            TabCommand::OpenTrack);  // keyevent("opentrack");
                                     // getMaster()->changeChild(tabsView);
    }
  }
}
...
void MainView::draw(QPainter *painter) {
  ...
  if (_currentView) {
    ...
    if (_currentView->getMaster() == 0) _currentView->setMaster(getMaster());
...
ConfigView::ConfigView() : GView() {
  ...
  sP = config.logsNames[2] + " " + std::to_string((int)*(config.logs[2])) +
       " press 1 to chng";
  _labC = std::make_unique<GLabel>(20, 120 + 200 - 55, sP.c_str());

  sP = config.logsNames[3] + " " + std::to_string((int)*(config.logs[3])) +
       " press 1 to chng";
...
void ConfigView::keyevent(std::string press) {
  ...
  if (press == "1") {
    ...
    *x = inv;
    ...
    std::string sP = config.logsNames[0] + " " +
                     std::to_string((int)*(config.logs[0])) +
                     " press 1 to chng";
  ...
  if (press == "2") {
    ...
    *x = inv;
    ...
    std::string sP = config.logsNames[1] + " " +
                     std::to_string((int)*(config.logs[1])) +
                     " press 2 to chng";
  ...
  if (press == "3") {
    ...
    *x = inv;
    ...
    std::string sP = config.logsNames[2] + " " +
                     std::to_string((int)*(config.logs[2])) +
                     " press 3 to chng";
  ...
  if (press == "4") {
    ...
    *x = inv;
    ...
    std::string sP = config.logsNames[3] + " " +
                     std::to_string((int)*(config.logs[3])) +
                     " press 4 to chng";
...
WelcomeView::WelcomeView() {
  _top = std::make_unique<GLabel>(50, 100, "Guitarmy - tablature compose tool",
                                  "", false);
  _mid = std::make_unique<GLabel>(
      50, 150, "This is early beta version - thank you for download", "",
      false);
...
TestsView::TestsView(MainView *mainV, TabView *tabV)
    : _upper(30, 30, "Start all tests!(dbl click)"),
...
void TestsView::openTestNumber(int num) {
  if (_tabsView->gotChanges() == false) {
    ...
    std::string fn = AConfig::getInst().testsLocation +
                     _buttons[num].getText().c_str() + ".gp4";
    // WIN? inverted?
...
QByteArray alphaCut(QByteArray &src) {
  ...
  for (int i = 0; i < src.size(); ++i) {
    ...
    // std::cout << "B1 = " << valueHi <<" "<<int(valueHi)<<"; B2 =
    // "<<valueLo<<" "<<int(valueLo)<<"; SOurce value ="<<letter<<std::endl;
  }
...
void InfoView::onclick(int x1, int y1) {
  if (_labelB->hit(x1, y1)) QMessageBox::aboutQt(0, "About Qt");
  ...
  if (_logShow->hit(x1, y1)) {
    QString crashName =
        AConfig::getInst().testsLocation.c_str() + QString("crashs.glog");
    if (QFile::exists(crashName)) {
      ...
      lines[2] = "Could be sent in ~ " +
                 std::to_string(allLogData.size() / 1000) + " iterations";
...
void BendInput::draw(QPainter *painter) {
  ...
  if (ptrToBend) {
    ...
    for (size_t i = 0; i < ptrToBend->size(); ++i) {
      ...
      int localY = vertical * -20;
      localY += 80 + 240;
      int localX = horizontal * 8;
      localX += 20;  // second op - shift in rect
...
void BendInput::keyevent(std::string press) {
  ...
  // also must have back button that will set bend value into note (instaed of
  // just escape it)
}
  ...
  if (_okButton->hit(x1, y1)) {
    if (ptrToNote) {
      if (ptrToBend) {
        ...
        // ptrToNote->getEffects().addPack(17,2,ptrToBend); //insure but it must
        // be already inside
      }
...
void BendInput::fillBend(int type, int height) {
  if (ptrToBend == 0)
    ptrToBend =
        new BendPoints;  // simple escape then could be reason of leak TODO
  ...
  if (type == 0) {
    ...
    p2.horizontal = 60;
    p2.vFlag = 0;
    ptrToBend->push_back(p0);
    ptrToBend->push_back(p1);
    ptrToBend->push_back(p2);
  }
  if (type == 1) {
    ptrToBend->clear();
    BendPoint p0;
    ...
    ptrToBend->push_back(p1);
    ptrToBend->push_back(p2);
    ptrToBend->push_back(p3);
    ptrToBend->push_back(p4);
  }
  if (type == 2) {
    ptrToBend->clear();
    BendPoint p0;
    ...
    ptrToBend->push_back(p3);
    ptrToBend->push_back(p4);
    ptrToBend->push_back(p5);
    ptrToBend->push_back(p6);
  }
  if (type == 3) {
    ptrToBend->clear();
    BendPoint p0;
        ...
        if (changes) {
          ...
          for (size_t i = 0; i < changes->size(); ++i) {
            ...
            std::string sX = "Type " + std::to_string(changeType) + "; Value " +
                             std::to_string(changeValue) + "; count " +
                             std::to_string(changeCount);
            ...
            if (changeType == 1) {
              getMaster()->setComboBox(0, "instruments", 220, 20, 150, 40,
                                       changeValue);
            ...
            if (changeType == 2) {
              getMaster()->setComboBox(6, "volume", 220, 120, 150, 40,
                                       changeValue);
              getMaster()->setComboBox(7, "changecombo3", 390, 120, 150, 40,
                                       -1);
              getMaster()->setComboBox(8, "changecombo4", 550, 120, 50, 40, 1);
            }
            if (changeType == 3) {
              getMaster()->setComboBox(9, "pan", 220, 170, 150, 40,
                                       changeValue);
              getMaster()->setComboBox(10, "changecombo3", 390, 170, 150, 40,
                                       -1);
              getMaster()->setComboBox(11, "changecombo4", 550, 170, 50, 40, 1);
            }
          }

          // getMaster()->SetButton(12,"back", 600, 20, 70,  30, "esc");
          // //refact
        }
      }
    }
...
void ChangesInput::turnOffChange(std::string combo) {
  ...
  if (combo == "2") awaitType = 1;
  if (combo == "5") awaitType = 8;
  if (combo == "8") awaitType = 2;
  if (combo == "11") awaitType = 3;
...
void ChangesInput::changeMainValue(int combo, int newValue) {
  ...
  if (combo == 0) awaitType = 1;
  if (combo == 3) awaitType = 8;
  if (combo == 6) awaitType = 2;
  if (combo == 9) awaitType = 3;
...
void ChangesInput::keyevent(std::string press) {
  if (press.substr(0, 4) == "com:") {
    ...
    if (combo == "0") changeMainValue(0, itemNum);
    if (combo == "3") changeMainValue(3, itemNum);
    if (combo == "6") changeMainValue(6, itemNum);
    if (combo == "9") changeMainValue(9, itemNum);

    if (combo == "2") changeSubValue(0, itemNum);
    if (combo == "5") changeSubValue(3, itemNum);
    if (combo == "8") changeSubValue(6, itemNum);
    if (combo == "11") changeSubValue(9, itemNum);
  }
}

void MainView::changeViewToLast() {
  if (_lastViews.empty()) return;

  GView *lastOne = _lastViews[_lastViews.size() - 1];
  _lastViews.pop_back();
  _currentView = lastOne;
  if (_dependent == false) _currentView->setUI();
}
...
void MainView::changeCurrentView(GView *newView) {
  ...
  if (_dependent == false) newView->setUI();
}

InfoView::InfoView() {
  int verticalShift = -60;  //-10; //40

  _labelA = std::make_unique<GLabel>(
      10, 100 + verticalShift,
      "Here is Guitarmy early beta test. This software is provided 'as-is', "
      "without any express or implied ");
  _labelA2 = std::make_unique<GLabel>(
      10, 125 + verticalShift,
      "warranty. In no event will the authors be held liable for any damages "
      "arising from the use of this software.");

  _icons = std::make_unique<GLabel>(
      10, 150 + verticalShift,
      "Some of icons used for app taken from icons8.com");

  _labelB = std::make_unique<GLabel>(
      10, 175 + verticalShift,
      "This software is developed using Qt library under LGPL licence. Press "
      "here for information");
  _labelC =
      std::make_unique<GLabel>(10, 200 + verticalShift,
                               "Link to the sources of Qt library used for "
                               "development(5.4.2): guitarmy.in/lgpl/src.zip");

  _labelD = std::make_unique<GLabel>(
      10, 225 + verticalShift,
      "Link to the licence text: "
      "http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html");
  _labelE = std::make_unique<GLabel>(
      10, 250 + verticalShift,
      "Address to contact author about LGPL topics: support@guitarmy.in");
  ...
  _sendCrash =
      std::make_unique<GLabel>(10, 360 + verticalShift, "Send crashed");
...
void TestsView::setUI() {
  for (size_t i = 0; i < _buttons.size(); ++i) {
    if (_mainView->getMaster())
      _mainView->getMaster()->SetButton(
          i, _buttons[i].getText(), _buttons[i].getX(), _buttons[i].getY(),
  ...
  if (_mainView->getMaster()) {
    ...
    _mainView->getMaster()->SetButton(
        i, _playlistButton->getText(), _playlistButton->getX(),
        _playlistButton->getY(), _playlistButton->getW(),
        _playlistButton->getH(), _playlistButton->getText());
    ++i;

    _mainView->getMaster()->SetButton(
        i, _stopPlaylist->getText(), _stopPlaylist->getX(),
        _stopPlaylist->getY(), _stopPlaylist->getW(), _stopPlaylist->getH(),
        _stopPlaylist->getText());
    ++i;

    _mainView->getMaster()->SetButton(
        i, _playlist2Button->getText(), _playlist2Button->getX(),
        _playlist2Button->getY(), _playlist2Button->getW(),
        _playlist2Button->getH(), _playlist2Button->getText());
  }
}
...
void TestsView::draw(QPainter *painter) {
  ...
  for (size_t i = 0; i < _buttons.size(); ++i) _buttons[i].draw(painter);
}
...
void ChordInput::setUI() {
  if (getMaster()) {
    ...
    getMaster()->setComboBox(2, "chordcombo3", 340, 20, 100, 40,
                             -1);  // 9 11 13 - not everywhere awailible
    getMaster()->setComboBox(3, "chordcombo4", 510, 20, 100, 40,
                             -1);  // _ +5 -5 - not everywhere

    // then only on 9 11 13

    getMaster()->setComboBox(4, "chordcombo5", 340, 70, 100, 40,
                             -1);  // _ +9 -9
    getMaster()->setComboBox(5, "chordcombo6", 510, 70, 100, 40,
                             -1);  // _ +11 -11
